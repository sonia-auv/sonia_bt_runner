...
BT::NodeStatus ComputeTrajectoryBetweenDetections::tick()
{
    ...
    RCLCPP_INFO(_ros_node->get_logger(),
                "ComputeTrajectoryBetweenDetections: midY=%.2f positionX=%.2f", midY, positionX);

    // Pose 1: drift laterally to the midpoint between the two detections.
    TrajectoryPose p1{};
    p1.positionY = midY;
    p1.frame = 1;

    // Pose 2: move forward through the gap.
    TrajectoryPose p2{};
