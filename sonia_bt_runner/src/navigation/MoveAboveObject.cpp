...
namespace navigation
{
    ...
    {
        MOVE_ABOVE_OBJECT_DETECTION_TYPE det;

        // We get the detected object by the AI
        getInput(MOVE_ABOVE_OBJECT_DETECTION, det);

        if(!getInput<MOVE_ABOVE_OBJECT_TRAJECTORY_TYPE>(MOVE_ABOVE_OBJECT_TRAJECTORY))
        {
        ...
        auto traj = getInput<MOVE_ABOVE_OBJECT_TRAJECTORY_TYPE>(MOVE_ABOVE_OBJECT_TRAJECTORY).value();
        traj.trajectory.push_back(t);
        setOutput<MOVE_ABOVE_OBJECT_TRAJECTORY_TYPE>(MOVE_ABOVE_OBJECT_TRAJECTORY, traj);
