...
namespace navigation
{
    ...
    BT::NodeStatus MoveToObject::tick()
    {
        ...
        auto traj = getInput<MOVE_TO_OBJECT_TRAJECTORY_TYPE>(MOVE_TO_OBJECT_TRAJECTORY_NAME).value();
        traj.trajectory.push_back(t1);
        traj.trajectory.push_back(t2);
        setOutput<MOVE_TO_OBJECT_TRAJECTORY_TYPE>(MOVE_TO_OBJECT_TRAJECTORY_NAME, traj);
