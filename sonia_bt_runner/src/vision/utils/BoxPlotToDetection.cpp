...
boxPlotToDetection (std::vector<sonia_common_ros2::msg::Detection>& detection_array)
{
    ...
    if (detection_array.size() % 4) {
        q1 = detection_array[(size_t)std::floor((float)detection_array.size()/4.0f)].distance_teta;
        q3 = detection_array[(size_t)std::floor((float)detection_array.size()*3.0f/4.0f)].distance_teta;
    } else {
...
if (detection_array.size() > 2) {
    ...
    // We compute the quartile
    if (detection_array.size() % 4) {
        q1 = detection_array[(size_t)std::floor((float)detection_array.size()/4.0f)].distance_beta;
        q3 = detection_array[(size_t)std::floor((float)detection_array.size()*3.0f/4.0f)].distance_beta;
    } else {
        q1 = (detection_array[(size_t)(detection_array.size()/4)].distance_beta + detection_array[(size_t)(detection_array.size()/4) + (size_t)1].distance_beta)/2.0f;
        q3 = (detection_array[(size_t)(detection_array.size()*3/4)].distance_beta + detection_array[(size_t)(detection_array.size()*3/4) + (size_t)1].distance_beta)/2.0f;
    }
